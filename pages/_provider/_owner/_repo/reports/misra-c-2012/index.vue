  ...
  <div class="flex flex-col gap-y-4">
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-400">
      <z-table class="border-none text-vanilla-100">
        <template #head>
          <z-table-row class="text-xs font-semibold uppercase tracking-wider text-vanilla-400 bg-ink-300">
            <z-table-cell class="mr-6 w-48 flex-initial text-left bg-ink-100"> Guideline </z-table-cell>
            ...
            <template v-if="complianceIssuesLoading">
              ...
              >
                <z-table-cell class="mr-6 w-48 flex-none text-left">
                  <div
                    class="-mx-2 h-full w-48 animate-pulse border-slate-400 opacity-50"
                  ></div>
                </z-table-cell>
...
import { ReportPageT, Dataset } from '~/types/reportTypes'
...
export default class Owasp extends mixins(RepoDetailMixin, ComplianceReportMixin) {
  ...
  async mounted(): Promise<void> {
    ...
    this.complianceIssues = complianceIssueList
     ...
     */
    await this.fetchReportBase(ReportLevel.Repository, this.repository.id, ReportPageT.MISRA_C)

    await Promise.all([
      this.fetchRecentStats(ReportLevel.Repository, this.repository.id, ReportPageT.MISRA_C),
