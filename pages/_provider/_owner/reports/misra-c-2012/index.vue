      ...
      <z-table class="cursor border-none text-vanilla-100">
        <template #head>
          <z-table-row class="text-xs font-semibold uppercase tracking-wider text-vanilla-400">
            <z-table-cell class="mr-6 w-48 flex-initial text-left bg-ink-200"> Guideline </z-table-cell>
            <z-table-cell class="text-left"> Category </z-table-cell>
        ...
        <template #body>
          <template v-if="complianceIssuesLoading">
            ...
            >
              <z-table-cell class="mr-6 w-48 flex-none text-left">
                <div
                  class="-mx-2 h-full w-48 animate-pulse border-slate-400 bg-ink-300 opacity-50"
                ></div>
              </z-table-cell>
          ...
          <template v-else>
            ...
            >
              <z-table-cell class="mr-6 w-48 flex-initial text-sm font-semibold text-vanilla-400">
                Rule {{ issue.issueId }}
              </z-table-cell>
...
import { ComplianceIssue, ComplianceIssueOccurrence, ReportLevel } from '~/types/types'
import { ReportPageT, Dataset, HistoricalValues } from '~/types/reportTypes'
...
const complianceIssueOccurrences = [
  {
    issueId: '10.2',
    title: 'Required',
...
export default class OwnerMisra extends mixins(OwnerDetailMixin, ComplianceReportMixin) {
  /**
  ...
  async mounted(): Promise<void> {
    this.complianceIssueList = complianceIssueOccurrences
     ...
     */
    await this.fetchReportBase(ReportLevel.Owner, this.owner.id, ReportPageT.MISRA_C)

    await Promise.all([
      this.fetchRecentStats(ReportLevel.Owner, this.owner.id, ReportPageT.MISRA_C),
      this.fetchComplianceIssues(ReportLevel.Owner, this.owner.id, ReportPageT.MISRA_C),
      this.setChartData()
    ])
  ...
  async setChartData(): Promise<void> {
    await this.fetchHistoricalValues(ReportLevel.Owner, this.owner.id, ReportPageT.MISRA_C)
    this.datasets = await getFormattedComplianceChartData(historicalData as unknown as HistoricalValues)
  }
}
</script>
