<template>
  <div class="flex h-screen flex-col justify-between text-center text-sm text-vanilla-400">
    <div class="my-24 grid flex-grow place-content-center space-y-8 px-4">
      <video
  ...
  async handleNextAction(): Promise<any> {
    ...
    switch (this.vcsResponse?.nextAction) {
      ...
      case NextActionChoice.GithubLogin:
        ...
        nextUrl =
          this.authUrls.find(({ provider }) => provider === VcsProviderChoices.Github)?.url || ''
        nextUrl && window.open(nextUrl)
        break
      default:
