...
describe('[Store] Issue/List', () => {
  ...
  */
  describe('[[Actions]]', () => {
    describe(`Action "${IssueListActions.FETCH_ISSUE_LIST}"`, () => {
      describe('Success', () => {
        beforeEach(async () => {
          localThis = {
            $providerMetaMap: {
          ...
          await actions[IssueListActions.FETCH_ISSUE_LIST].call(localThis, actionCxt, {
            provider: 'gh',
        ...
        test(`successfully commits mutation ${IssueListMutations.SET_LOADING}`, () => {
          ...
          expect(firstCall[0]).toEqual(IssueListMutations.SET_LOADING)

          // Assert if right data is passed to the mutation.
          expect(firstCall[1]).toEqual(true)

          // Assert if `MUT_SET_LOADING` is being commited or not.
          expect(thirdCall[0]).toEqual(IssueListMutations.SET_LOADING)

          // Assert if right data is passed to the mutation.
          expect(thirdCall[1]).toEqual(false)
        })

        test(`successfully commits mutation ${IssueListMutations.SET_ISSUE_LIST}`, async () => {
          const {
            mock: {
              calls: [, secondCall]
            }
          ...
          expect(secondCall[0]).toEqual(IssueListMutations.SET_ISSUE_LIST)
      ...
      describe('Failure', () => {
        ...
        test(`successfully commits mutation ${IssueListMutations.SET_ERROR}`, () => {
          ...
          expect(secondCall[0]).toEqual(IssueListMutations.SET_ERROR)
  ...
  */
  describe('[[Mutations]]', () => {
    describe(`Mutation "${IssueListMutations.SET_LOADING}"`, () => {
      test('successfully updates loading field in state', () => {
        mutations[IssueListMutations.SET_LOADING](issueListState, true)
        expect(issueListState.loading).toEqual(true)
      })
    })

    describe(`Mutation "${IssueListMutations.SET_ERROR}"`, () => {
      test('successfully updates loading field in state', () => {
        const dummyError = {
          graphQLErrors: {
            message: 'Dummy error',
        ...
        mutations[IssueListMutations.SET_ERROR](issueListState, dummyError)
        expect(issueListState.error).toEqual(dummyError)
      })
    })

    describe(`Mutation "${IssueListMutations.SET_ISSUE_LIST}"`, () => {
      beforeEach(() => {
      ...
      test('successfully adds new issue list to the state', () => {
        ...
        mutations[IssueListMutations.SET_ISSUE_LIST](issueListState, newIssueList)
        expect(issueListState.issueList).toEqual(newIssueList)
      })

      test('successfully appends data', () => {
        const newIssueList: RepositoryIssueConnection = mockIssueListState()
          .issueList as RepositoryIssueConnection

        if (newIssueList.edges[0]?.node?.issueType) {
          newIssueList.edges[0].node.issueType = 'antipattern'
        }
        mutations[IssueListMutations.SET_ISSUE_LIST](issueListState, newIssueList)
        expect(issueListState.issueList.edges[0]?.node?.issueType).toEqual(
          newIssueList.edges[0]?.node?.issueType
        )
      })
    })
  })
