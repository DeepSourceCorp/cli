    ...
    describe(`Action "${IssueDetailActions.IGNORE_ISSUE_CHECK_ISSUE}"`, () => {
      describe('Success', () => {
        ...
        test(`successfully commits mutation ${IssueDetailMutations.SET_LOADING}`, async () => {
          ...
          // Assert if `IssueDetailMutations.SET_LOADING` is being commited or not.
          expect(secondCall[0]).toEqual(IssueDetailMutations.SET_LOADING)
      ...
      describe('Failure', () => {
          ...
          localThis = {
            ...
            async $applyGraphqlMutation(): Promise<Error> {
              return new Promise<Error>((resolve, reject) =>
                reject(new Error('IssueDetailMutations.ERR1'))
              )
            }
          }
        ...
        test(`successfully commits mutation ${IssueDetailMutations.SET_ERROR}`, async () => {
          ...
          // Assert if the payload passed to the mutation was empty.
          expect(secondCall[1]).toEqual(Error('IssueDetailMutations.ERR1'))
        })
      })
    })
