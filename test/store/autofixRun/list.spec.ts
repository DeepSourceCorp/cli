import { state, mutations, actions, MUT_SET_ERROR, MUT_SET_LOADING, AutofixRunListActionContext, AutofixRunListModuleState, ACT_FETCH_AUTOFIX_RUN_LIST, MUT_SET_AUTOFIX_RUN_LIST } from '~/store/autofixRun/list';
...
describe('[Store] AutofixRun/List', () => {
  ...
  */
  describe('[[Actions]]', () => {
    describe(`Action "${ACT_FETCH_AUTOFIX_RUN_LIST}"`, () => {
      describe(`Success`, () => {
        beforeEach(async () => {
          localThis = {
            $providerMetaMap: {
          ...
          await actions[ACT_FETCH_AUTOFIX_RUN_LIST].call(localThis, actionCxt, {
            provider: 'gh',
        ...
        test(`successfully commits mutation ${MUT_SET_AUTOFIX_RUN_LIST}`, async () => {
          const { mock: { calls: [, secondCall,] } } = commit
          const apiResponse = await localThis.$fetchGraphqlData()

          // Assert if `MUT_SET_AUTOFIX_RUN_LIST` is being commited or not.
          expect(secondCall[0]).toEqual(MUT_SET_AUTOFIX_RUN_LIST)
  ...
  describe('[[Mutations]]', () => {
    describe(`Mutation "${MUT_SET_LOADING}"`, () => {
      test('successfully updates loading field in state', () => {
        mutations[MUT_SET_LOADING](autofixRunListState, true)
    ...
    describe(`Mutation "${MUT_SET_ERROR}"`, () => {
      test('successfully updates loading field in state', () => {
        ...
        mutations[MUT_SET_ERROR](autofixRunListState, dummyError)
        expect(autofixRunListState.error).toEqual(dummyError)
      })
    })

    describe(`Mutation "${MUT_SET_AUTOFIX_RUN_LIST}"`, () => {
      beforeEach(() => {
      ...
      test('successfully adds new run list to the state', () => {
        ...
        mutations[MUT_SET_AUTOFIX_RUN_LIST](autofixRunListState, newAutofixRunList)
        expect(autofixRunListState.autofixRunList).toEqual(newAutofixRunList)
      })

      test('successfully appends data', () => {
        const newAutofixRunList: AutofixRunConnection = mockAutofixRunListState().autofixRunList as AutofixRunConnection;

        if (newAutofixRunList.edges[0]?.node?.filesAffected) {
          newAutofixRunList.edges[0].node.filesAffected = 2
        }
        mutations[MUT_SET_AUTOFIX_RUN_LIST](autofixRunListState, newAutofixRunList)
        expect(autofixRunListState.autofixRunList.edges[0]?.node?.filesAffected).toEqual(newAutofixRunList.edges[0]?.node?.filesAffected)
      })
    })
  })
});
