import { state, mutations, actions, MUT_SET_ERROR, MUT_SET_LOADING, AutofixRunDetailActionContext, AutofixRunDetailModuleState, ACT_FETCH_AUTOFIX_RUN, MUT_SET_AUTOFIX_RUN } from '~/store/autofixRun/detail';
...
describe('[Store] AutofixRun/Detail', () => {
  ...
  */
  describe('[[Actions]]', () => {
    describe(`Action "${ACT_FETCH_AUTOFIX_RUN}"`, () => {
      describe(`Success`, () => {
        beforeEach(async () => {
          localThis = {
            $providerMetaMap: {
          ...
          await actions[ACT_FETCH_AUTOFIX_RUN].call(localThis, actionCxt, {
            runId: 'string'
          })
        })
        ...
        test(`successfully commits mutation ${MUT_SET_AUTOFIX_RUN}`, async () => {
          const { mock: { calls: [, secondCall,] } } = commit
          const apiResponse = await localThis.$fetchGraphqlData()

          // Assert if `MUT_SET_AUTOFIX_RUN` is being commited or not.
          expect(secondCall[0]).toEqual(MUT_SET_AUTOFIX_RUN)
  ...
  describe('[[Mutations]]', () => {
    describe(`Mutation "${MUT_SET_LOADING}"`, () => {
      test('successfully updates loading field in state', () => {
        mutations[MUT_SET_LOADING](autofixRunDetailState, true)
    ...
    describe(`Mutation "${MUT_SET_ERROR}"`, () => {
      test('successfully updates loading field in state', () => {
        ...
        mutations[MUT_SET_ERROR](autofixRunDetailState, dummyError)
        expect(autofixRunDetailState.error).toEqual(dummyError)
      })
    })

    describe(`Mutation "${MUT_SET_AUTOFIX_RUN}"`, () => {
      beforeEach(() => {
      ...
      test('successfully adds new run detail to the state', () => {
        ...
        mutations[MUT_SET_AUTOFIX_RUN](autofixRunDetailState, newAutofixRun)
