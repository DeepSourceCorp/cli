...
describe('[Store] Integrations/List', () => {
  ...
  describe('[[ Actions ]]', () => {
    ...
    test(`Action "${IntegrationsListActions.FETCH_INTEGRATIONS_WITH_DETAILS}"`, async () => {
      const localThis = {
        async $fetchGraphqlData(): Promise<GraphqlQueryResponse> {
          return new Promise<GraphqlQueryResponse>((resolve) =>
            setTimeout(() => resolve({ data: { integrations: integrationsWithDetails } }), 10)
          )
        }
      }
      ...
      // Dispatch the Vuex action
      await actions[IntegrationsListActions.FETCH_INTEGRATIONS_WITH_DETAILS].call(
        localThis as any,
        actionCxt,
        {
      ...
      expect(actionCxt.commit).toHaveBeenCalledWith(
        IntegrationsListMutations.SET_INTEGRATIONS,
        integrationsWithDetails
      )
    })

    test(`Action "${IntegrationsListActions.FETCH_INTEGRATIONS_WITH_DETAILS}" error handler`, async () => {
      const localThis = {
        async $fetchGraphqlData(): Promise<void> {
          return new Promise<void>((_, reject) => setTimeout(() => reject(), 10))
        },
